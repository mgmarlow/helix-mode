...
(defvar helix-normal-state-keymap
  (let ((keymap (make-keymap)))
    ...
    ;; Movement keys
    (keymap-set keymap "h" #'helix-backward-char)
    (keymap-set keymap "l" #'helix-forward-char)
    (keymap-set keymap "j" #'helix-next-line)
    (keymap-set keymap "k" #'helix-previous-line)
    (keymap-set keymap "w" #'helix-forward-word)
    (keymap-set keymap "b" #'helix-backward-word)

    ;; Go-to menu
    (keymap-set keymap "g" 'helix-goto-map)
    (keymap-set helix-goto-map "l" #'helix-go-end-line)
    (keymap-set helix-goto-map "h" #'helix-go-beginning-line)
    (keymap-set helix-goto-map "g" #'helix-go-beginning-buffer)
    (keymap-set helix-goto-map "e" #'helix-go-end-buffer)
    (keymap-set helix-goto-map "j" #'helix-next-line)
    (keymap-set helix-goto-map "k" #'helix-previous-line)
    
    ;; Editing commands
    (keymap-set keymap "x" #'helix-select-line)
    (keymap-set keymap "d" #'helix-kill-thing-at-point)
    (keymap-set keymap "y" #'kill-ring-save)
    (keymap-set keymap "p" #'yank)
    (keymap-set keymap "v" #'helix-begin-selection)
    (keymap-set keymap "u" #'undo)
    (keymap-set keymap "o" #'helix-insert-newline)
    (keymap-set keymap "O" #'helix-insert-prevline)
    (keymap-set keymap "/" #'helix-search)
    (keymap-set keymap "n" #'helix-search-forward)
    (keymap-set keymap "N" #'helix-search-backward)
    (keymap-set keymap "r" #'helix-replace)
    (keymap-set keymap "R" #'helix-replace-yanked)

    ;; State switching
    (keymap-set keymap "i" #'helix-insert)
    (keymap-set keymap "I" #'helix-insert-beginning-line)
    (keymap-set keymap "a" #'helix-insert-after)
    (keymap-set keymap "A" #'helix-insert-after-end-line)
    (keymap-set keymap ":" #'helix-execute-command)
    (keymap-set keymap "ESC" #'keyboard-quit)
    (keymap-set keymap "DEL" (lambda () (interactive)))
    keymap)
  "Keymap for Helix normal state.")

(defvar helix-insert-state-keymap
  (let ((keymap (make-keymap)))
    (keymap-set keymap "ESC" #'helix-insert-exit)
    keymap)
